...
http://ip:port/sendPic?siteId=11&deviceId=1&airlineKey=AAAA&detectTimeCur=20250701125959
...
| airlineKey | String | 当前航线唯一标识 |
| detectTimeCur | String | 检测时间，格式：20250701125959 |
| file | 文件 | 单张图片文件，使用 multipart/form-data 上传 |

说明：

| 属性名 | 类型 | 描述 |
| --- | --- | --- |
| file | 文件 | 文件名由无人机端按同一 `detectTimeCur` 批次内 `1、2、3... + 原始扩展名` 上传，例如 `1.jpg`、`2.jpg` |

前端服务器保存规则：

| 规则 | 描述 |
| --- | --- |
| 目录 | `D:/tycho/imgs/<deviceId>/YYYY/MM/DD/<detectTimeCur>/` |
| 日期来源 | `YYYY/MM/DD` 由 `detectTimeCur` 推导 |
| 落盘方 | 前端服务器根据 `deviceId` 与 `detectTimeCur` 负责创建目录并保存文件 |
| 文件示例 | 第 1 张图片可保存为 `.../<detectTimeCur>/1.png`，第 2 张可保存为 `.../<detectTimeCur>/2.png` |
